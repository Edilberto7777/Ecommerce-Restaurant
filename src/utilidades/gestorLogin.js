...
const gestionarLogin = (event, datos, navigate, setIsLoggedIn, setMunicipioActivo, telefono, setTelefono, password, usuario, cantContenido, setUsuario, setPrecioMensajeria) => {
  if (datos.usuario && datos.password) {
    datosEstadisticas(cantContenido, usuario, password, telefono, setTelefono, setMunicipioActivo, setIsLoggedIn, event, navigate, datos, setUsuario, setPrecioMensajeria);
  } else {
...
export const gestionadorSubmit = (
  event,
  cantContenido,
  contador,
  setContador,
  ...
  if (cantContenido == 2) {
    gestionarLogin(event, datos, navigate, setIsLoggedIn, setMunicipioActivo, telefono, setTelefono, password, usuario, cantContenido, setUsuario, setPrecioMensajeria, setAdminLoggueada);
  } else {
    // Si es registro → guardar usuario
    gestionarRegistro(event, datos, navigate, setIsLoggedIn, setMunicipioActivo, telefono, setTelefono, password, usuario, cantContenido, setUsuario, setPrecioMensajeria, setAdminLoggueada);
  }
